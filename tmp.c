...
{
	t_camera	*cam;          // Pointeur vers la caméra
	double		ray_dir_x;     // Direction du rayon
	double		ray_dir_y;
	int			map_x;         // Position sur la grille
	int			map_y;
...
void	calculate_step_and_side_dist(t_ray *ray)
{
	...
	{
		ray->step_x = 1;
		ray->side_dist_x = (ray->map_x + 1.0 - ray->cam->pos_x) * ray->delta_dist_x;
	}
	...
	{
		ray->step_y = 1;
		ray->side_dist_y = (ray->map_y + 1.0 - ray->cam->pos_y) * ray->delta_dist_y;
	}
}
...
void	calculate_line_height(t_ray *ray)
{
	...
	if (ray->side == 0)
		ray->perp_wall_dist = (ray->map_x - ray->cam->pos_x + (1 - ray->step_x) / 2) / ray->ray_dir_x;
	else
		ray->perp_wall_dist = (ray->map_y - ray->cam->pos_y + (1 - ray->step_y) / 2) / ray->ray_dir_y;
...
int	get_wall_color(t_ray *ray)
{
	...
	// Choisir la couleur selon la direction du mur
	if (ray->side == 0) // Murs Nord-Sud
	{
		if (ray->step_x == 1)
			color = 0xFF0000; // Rouge - mur Est
		else
			color = 0x00FF00; // Vert - mur Ouest
	}
	else // Murs Est-Ouest
	{
		if (ray->step_y == 1)
			color = 0x0000FF; // Bleu - mur Sud
		else
			color = 0xFFFF00; // Jaune - mur Nord
	}
	
	// Assombrir les murs Est-Ouest pour donner un effet d'ombrage
	if (ray->side == 1)
		color = color / 2;
...
void	draw_ceiling_and_floor(t_cub3d *cub3d, int x, int draw_start, int draw_end)
{
	int y;
	int ceiling_color;
	int floor_color;
	
	ceiling_color = (cub3d->game->roof->r << 16) | 
					(cub3d->game->roof->g << 8) | 
					cub3d->game->roof->b;
					
	floor_color = (cub3d->game->floor->r << 16) | 
				  (cub3d->game->floor->g << 8) | 
				  cub3d->game->floor->b;
...
void	render_frame(t_cub3d *cub3d)
{
	...
	while (x < WIN_WIDTH)
	{
		...
		// Calculer les paramètres DDA
		calculate_step_and_side_dist(&ray);
		
		// Effectuer l'algorithme DDA
